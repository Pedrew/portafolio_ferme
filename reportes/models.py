...
class Reportes(models.Model):
    ...
    def comprasUsuario(self):
        con = cx_Oracle.connect('admin/admin123@dbdrew.cteemzssmjhk.sa-east-1.rds.amazonaws.com/DBDREW')
        cur = con.cursor()
        cur.execute('select count(b.id_usuario), u.nombre, u.apellido from boleta b join usuarios u on b.id_usuario = u.user_id group by b.id_usuario, u.nombre, u.apellido')            
        res = cur.fetchall()
        column = [row[0] for row in cur.description]
        array = []
        for r in res:
